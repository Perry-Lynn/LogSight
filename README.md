...
- **兼容常见部署方式**：支持密码、私钥、SSH config、SSH agent，以及 keyboard-interactive/MFA 场景；可浏览远程目录并校验路径权限。
...
| 能力 | 说明 |
| --- | --- |
| 远程连接 | SSH 密码、私钥、SSH config、SSH agent |
...
- SSH 凭据不会写入 Git 仓库；应用会在本机存储加密后的敏感字段，当前实现使用 AES-256-GCM 和 Argon2id。
- 日志内容只在本地界面和 SSH 会话中处理；项目没有内置遥测或日志上传接口。
- 路径会在执行远程命令前做基础校验，并使用 shell quoting 降低命令注入风险。
- 当前凭据存储仍属于桌面应用的 MVP 实现，尚未经过独立安全审计。生产环境请使用最小权限 SSH 账号、只读日志权限和短期凭据，并先在隔离环境验证。
