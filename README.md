...
远程服务器需要 SSH 服务，并且当前日志流、历史查询和日志源探测依赖 POSIX/Unix 命令（包括 `tail`、`grep`、`awk`、`find` 和 GNU `stat`）。Windows 远程服务器适配将在后续阶段实现。
...
- 测试连接会额外探测远程系统、Shell 和日志命令能力；SSH 能连通不代表当前日志模式一定可用。
...
- 如果系统钥匙串丢失，应用可以重建本地密钥，但旧密码和私钥密文无法恢复；服务器地址等非敏感配置会保留，需重新录入凭据。
- 导入服务器 JSON 时会创建新的本地服务器记录，禁用导入文件中的自动脚本；跨设备导入且无法用当前主密钥解密的凭据需要重新录入。
